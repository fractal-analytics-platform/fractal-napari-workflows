...
import napari_workflows  # noqa: TC002
import numpy as np
import pandas as pd
import zarr
    ...
    *,
    ...
    Args:
        ...
        input_ROI_table: Name of the ROI table over which the task loops to
            ...
            Examples include `FOV_ROI_table` => loop over the field of views;
            `organoid_ROI_table` => loop over the organoid ROI table
            (generated by another task); `well_ROI_table` => process the whole
            well as one image.
    ...
    if is_measurement_only_workflow and relabeling:
        logger.warning(
            "This is a measurement-output-only workflow, setting relabeling=False."
        )
        relabeling = False
    if relabeling:
    ...
    logger.info(
        f"Completed reading ROI table {input_ROI_table}, found {num_ROIs} ROIs."
    )
    ...
    if image_inputs:
        ...
        for name, params in image_inputs:
            ...
            if expected_dimensions == 3 and shape[0] == 1:
                logger.warning(
                    f"Input {name} has shape {shape} but {expected_dimensions=}"
                )
            if expected_dimensions == 2:
                if len(shape) == 2:
                    ...
                    msg = f"Input {name} has shape {shape} but {expected_dimensions=}"
                    logger.error(msg)
                    raise ValueError(msg)
    ...
    if label_outputs:
        ...
        if len(label_shape) == 2 and len(label_chunksize) == 2:
            if expected_dimensions == 3:
                raise ValueError(
                    f"Something wrong: {label_shape=} but {expected_dimensions=}"
                )
        ...
        for name, out_params in label_outputs:
            ...
            new_datasets = rescale_datasets(
                datasets=[
                    ds.model_dump() for ds in ngff_image_meta.multiscale.datasets
                ],
                coarsening_xy=coarsening_xy,
    ...
    for _name, out_params in label_outputs:
        label_name = out_params.label_name
        build_pyramid(
