...
from ._validation import (
    check_file_number,
    validate_axes_and_coordinateTransformations,
    validate_labels_and_measurements,
    validate_schema,
)
from ._zenodo_ome_zarrs import prepare_2D_zarr, prepare_3D_zarr
...
def test_napari_workflow(
    ...
    input_specs: dict[str, NapariWorkflowsInput] = {
        "input": {"type": "image", "channel": {"wavelength_id": "A01_C01"}},  # type: ignore
    }
    output_specs: dict[str, NapariWorkflowsOutput] = {
        "Result of Expand labels (scikit-image, nsbatwm)": {  # type: ignore
            "type": "label",
    ...
    input_specs = {
        "dapi_img": {"type": "image", "channel": {"wavelength_id": "A01_C01"}},  # type: ignore
        "dapi_label_img": {"type": "label", "label_name": "label_DAPI"},  # type: ignore
    }
    output_specs = {
...
def test_napari_worfklow_label_input_only(
    ...
    input_specs = {
        "test_labels": {"type": "label", "label_name": "label_DAPI"},  # type: ignore
    }
    output_specs = {
        "Result of Expand labels (scikit-image, nsbatwm)": {  # type: ignore
            "type": "label",
...
def test_relabeling(
    ...
    if needs_labels:
        workflow_file = str(testdata_path / "napari_workflows" / RELABELING_CASE_1[0])
        for zarr_url in zarr_urls:
            napari_workflows_wrapper(
                zarr_url=zarr_url,
    ...
    workflow_file = str(testdata_path / "napari_workflows" / workflow_file_name)
    debug(workflow_file)
    debug(input_specs)
    debug(output_specs)
    for zarr_url in zarr_urls:
    ...
    if dataframe_outputs:
        meas = ad.read_zarr(Path(zarr_urls[0], f"tables/{TABLE_NAME}/").as_posix())
        debug(meas.var_names)
        assert "area" in meas.var_names
        assert "bbox_area" in meas.var_names
...
def test_napari_workflow_empty_input_ROI_table(
    ...
    # Load measurements
    meas = ad.read_zarr(Path(zarr_urls[0], "tables/regionprops_DAPI/").as_posix())
    debug(meas.var_names)
